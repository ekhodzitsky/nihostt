...
//!
//! Native Rust replacement for external Python scripts. Auto-invoked after
//! `nihostt download` and `nihostt serve` (see `src/main.rs`); also exposed
//! as the `nihostt quantize` subcommand.
//!
...
//! 0.14, so we lean on the generated accessor methods (`data_type()`,
...
mod tests {
    ...
    fn test_roundtrip_encode_decode_minimal_model() {
        ...
        let model = ModelProto {
            ir_version: Some(8),
            producer_name: Some("nihostt-test".into()),
        ...
        assert_eq!(decoded.producer_name(), "nihostt-test");
